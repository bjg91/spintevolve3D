 ...
 * 22/06/2007
 * 
 * Edited to allow for 3D spin polarized current 
 * Brian Gould 
 */
...
OXS_EXT_REGISTER(Anv_SpinTEvolve3D);

/* End includes */

// Constructor
Anv_SpinTEvolve3D::Anv_SpinTEvolve3D(
    ...
    rkstep_ptr(NULL)
{
	...
	if(HasInitValue("u")) {
    OXS_GET_INIT_EXT_OBJECT("u",Oxs_VectorField,u_init); //BRIAN: This code imports u
                                                         //u is the coefficient for the STT terms; dm/dt = ... + u.m x m x dm/dx + beta u.m x dm/dx
                                                         //We need to make u an Oxs_VectorField object to allow current to travel in the y and z directions 
                                                         // or have seperate calls for u_x, u_y, and u_z

    } else {
    // Default is zero
    Oxs_Ext* foo=MakeNew("Oxs_UniformVectorField",director,"value 0.0 0.0 0.0"); //Potential ERROR, Initialize vector field 
    u_init.SetAsOwner(dynamic_cast<Oxs_VectorField*>(foo));//BRIAN: Again, change to Oxs_VectorField FIXED
    }
  ...
  if(method.compare("rk2")==0) {
    rkstep_ptr = &Anv_SpinTEvolve3D::TakeRungeKuttaStep2;
  } else if(method.compare("rk4")==0) {
    rkstep_ptr = &Anv_SpinTEvolve3D::TakeRungeKuttaStep4;
  } else if(method.compare("rkf54m")==0) {
    rkstep_ptr = &Anv_SpinTEvolve3D::TakeRungeKuttaFehlbergStep54M;
  } else if(method.compare("rkf54s")==0) {
    rkstep_ptr = &Anv_SpinTEvolve3D::TakeRungeKuttaFehlbergStep54S;
  } else if(method.compare("rkf54")==0) {
    rkstep_ptr = &Anv_SpinTEvolve3D::TakeRungeKuttaFehlbergStep54;
  } else {
    throw Oxs_Ext::Error(this,"Invalid initialization detected:"
  ...
  max_dm_dt_output.Setup(this,InstanceName(),"Max dm/dt","deg/ns",0,
     &Anv_SpinTEvolve3D::UpdateDerivedOutputs);
  ...
  ave_u_output.Setup(this,InstanceName(),"average u","m/s",0,
		 &Anv_SpinTEvolve3D::UpdateSpinTorqueOutputs);
...
OC_BOOL Anv_SpinTEvolve3D::Init()
{
  // Setup outputs
...
//	u.Release(); //Potential ERROR: does ThreeVector class have Release() ? 
...
void Anv_SpinTEvolve3D::UpdateMeshArrays(const Oxs_Mesh* mesh)
{
  ...
  if(rmesh==NULL) {
    String msg="Import mesh to Anv_SpinTEvolve3D::UpdateMeshArrays"
      " is not an Oxs_RectangularMesh object.";
    throw Oxs_Ext::Error(msg.c_str());
  }
  Xstep = rmesh->EdgeLengthX(); //BRIAN: add y and z FIXED
	n_x = rmesh->DimX();
  ...
  Zstep = rmesh->EdgeLengthZ();
	...
	u_init->FillMeshValue(mesh,u); //BRIAN: I think this makes u a property of the mesh, allowing us to call u[i] like we would call another mesh property (ie spin_[i]) 
  ...
  ave_u = 0.0;
  for(i=0;i<size;i++) {
    ave_u += sqrt(u[i]*u[i]); //BRIAN: Need to fix if u is a vector FIXED
	}
  ...
  for(j=0;j<fixed_count;j++) {
    OC_INDEX i = GetFixedSpin(j);
		u[i].Set(0.0, 0.0, 0.0); //ERROR: cant set vector with scalar FIXED
  }
...
OC_REAL8m Anv_SpinTEvolve3D::EvaluateuProfileScript
(OC_UINT4m stage,
...
void Anv_SpinTEvolve3D::Calculate_dm_dt //BRIAN: This is where the LLG equation is coded. u is the coefficient for the STT terms; dm/dt = ... + u.m x m x dm/dx + beta u.m x dm/dx
...
{ // Imports: state, mxH_, pE_pt_
  ...
  //   to be the same as mxH_.  In this case, mxH_ 
  ...
  const OC_INDEX size = mesh->Size(); // Assume import data are compatible
    ...
    ThreeVector xhat(1., 0., 0.);
    ThreeVector yhat(0., 1., 0.);
  ...
  for(i=0;i<size;i++) { //BRIAN: This is the loop over the mesh. 
                        // Suppose we are interested in the spin at a cell with cooordinates x, y, z. The author of this code calls a spin of a certain cell with spin_[i] where 
                        // i = x + num_cells_in_x_row * y + num_cells_in_x_row * num_cells_in_y_row * z
                        // The previous statement is evidenced by the structure of the code that calculates dm/dx
                        // This makes it much easier to calculate dm/dx as opposed to dm/dy or dm/dz since a loop over the cells needs to only consider the ith and ith+1 cells for dm/dx
                        // as opposed to the ith and (i+num_cells_in_x_row)th cells for dm/dy
                        // size: the total number of cells in the OOMMF simulation 
                        // n_x: the number of cells in the x row
    ...
    if(Ms_[i]==0) {
      dm_dt_[i].Set(0.0,0.0,0.0); 
        ...
        OC_INDEX y = 0;
        OC_INDEX z = 0;
        x = i % n_x;// BRIAN: See how x is calculated from i FIXED
        y = (i / n_x) % n_y;
        z = i / (n_x * n_y);
        ...
        //BRIAN: This is where dm/dx is calculated (refered to as gradx_m)

        ThreeVector base = spin_[i];
        ThreeVector gradi_m(0., 0., 0.);
        ThreeVector ugrad_m(0., 0., 0.);
        ThreeVector m_ugrad_m(0., 0., 0.);
        ThreeVector m_m_ugrad_m(0., 0., 0.);
        ThreeVector inter_dEdt(0., 0., 0.);
        OC_REAL8m u_x = xhat * u[i];
        OC_REAL8m u_y = yhat * u[i];
        OC_REAL8m u_z = zhat * u[i];


        if (x > 0) {
            j = i - 1;
            if (Ms_[j] != 0.0) {
                gradi_m = 0.5 * (base - spin_[j]);
            }
        }
        if (x < n_x - 1) {
            j = i + 1;
            if (Ms_[j] != 0.0) {
                gradi_m += 0.5 * (spin_[j] - base);
            }
        }
        ...
        gradi_m *= (1 / Zstep);
        gradi_m *= u_z;// gradi_m=u*pm_pz 



        ugrad_m += gradi_m;

        //BRIAN: This is where dm/dx is multiplied by the coefficient u. 
        //This code expresses u as a scalar field, and we must make u a vector field if we want a 2D or 3D current distribution. 
        // u[i] returns the value of u at the i'th cell and is a scalar. We would want u[i] to return a ThreeVector
        //Alternatively, we could preserve this code and make ux, uy, and uz seperate arguements. This may be easier to code but would be more difficult for the user. 

        ugrad_m *= umult; // BRIAN: This umult term is just a term that allows the user to add time dependence. We don't have to ajdust it. 
        m_ugrad_m = spin_[i];
        m_ugrad_m ^= ugrad_m; // u*m^pm_px

        m_m_ugrad_m = m_ugrad_m;
        m_m_ugrad_m ^= spin_[i]; //u*(m^pm_px)^m=-u*m^(m^pm_px)=u*pm_px

        //BRIAN: This is where the LLG equation is coded 
        ...
        // BRIAN: I believe everything past this point in the loop over the mesh is not necessary for us to adjust 

        OC_REAL8m dm_dt_sq = dm_dt_[i].MagSq();
        if (dm_dt_sq > 0) {
            inter_dEdt -= mxH;
            inter_dEdt *= fabs(cell_alpha * cell_mgamma); // inter_dEdt=-alpha*|gamma_LL|*m^H
            m_m_ugrad_m *= (cell_alpha - beta) / (1 + cell_alpha * cell_alpha); //m_m_ugrad_m=(beta-alpha)/(1+alpha^2)*u*m^(m^pm_px)
            inter_dEdt -= m_m_ugrad_m; //there was an error of sign here
            m_ugrad_m *= (cell_alpha * beta + 1) / (1 + cell_alpha * cell_alpha); //m_ugrad_m=(1+alpha*beta)/(1+alpha^2)*u*m^pmpx
            inter_dEdt += m_ugrad_m; //inter_dEdt=-alpha*|gamma_LL|*m^H+(beta-alpha)/(1+alpha^2)*u*pm_px+(1+alpha*beta)/(1+alpha^2)*u*m^pmpx
...
void Anv_SpinTEvolve3D::CheckCache(const Oxs_SimState& cstate)
{
  ...
  if(!cache_good) {
    throw Oxs_Ext::Error(this,
       "Anv_SpinTEvolve3D::CheckCache: Invalid data cache.");
  }
}


void
Anv_SpinTEvolve3D::AdjustState
(OC_REAL8m hstep,
 OC_REAL8m mstep,
 ...
 OC_REAL8m& norm_error) const
{
  ...
  // to Anv_SpinTEvolve3D::Step() takes 2 half-steps, and it is the
  ...
  // in by the Anv_SpinTEvolve3D::NegotiateTimeStep() method.
}


void Anv_SpinTEvolve3D::UpdateTimeFields
...
void Anv_SpinTEvolve3D::NegotiateTimeStep
...
{ // This routine negotiates with driver over the proper step size.
  ...
  if(!cstate.GetDerivedData("Max dm/dt",max_dm_dt)) {
    throw Oxs_Ext::Error(this,
       "Anv_SpinTEvolve3D::NegotiateTimeStep: max_dm_dt not cached.");
  }
  ...
  if(!cstate.GetDerivedData("Timestep lower bound",
			    timestep_lower_bound)) {
    throw Oxs_Ext::Error(this,
       "Anv_SpinTEvolve3D::NegotiateTimeStep: "
...
void Anv_SpinTEvolve3D::TakeRungeKuttaStep2
(OC_REAL8m stepsize,
...
{ // This routine performs a second order Runge-Kutta step, with
  ...
  if(!endstate->AddDerivedData("Timestep lower bound",
    ...
    throw Oxs_Ext::Error(this,
			 "Anv_SpinTEvolve3D::TakeRungeKuttaStep2:"
...
void Anv_SpinTEvolve3D::TakeRungeKuttaStep4
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Anv_SpinTEvolve3D::RungeKuttaFehlbergBase54
(RKF_SubType method,
 OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
...
{ // Runge-Kutta-Fehlberg routine with combined 4th and 5th
  ...
  default:
    throw Oxs_Ext::Error(this,
		 "Anv_SpinTEvolve3D::RungeKuttaFehlbergBase54:"
...
void Anv_SpinTEvolve3D::TakeRungeKuttaFehlbergStep54
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Anv_SpinTEvolve3D::TakeRungeKuttaFehlbergStep54M
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Anv_SpinTEvolve3D::TakeRungeKuttaFehlbergStep54S
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
OC_REAL8m Anv_SpinTEvolve3D::MaxDiff
...
void Anv_SpinTEvolve3D::AdjustStepHeadroom(OC_INT4m step_reject)
...
OC_BOOL
Anv_SpinTEvolve3D::Step(const Oxs_TimeDriver* driver,
                      Oxs_ConstKey<Oxs_SimState> current_state_key,
                      ...
                      Oxs_Key<Oxs_SimState>& next_state_key)
{
  ...
  if(next_timestep<=0.0 ||
    ...
    if(cstate.stage_number==0
      ...
      throw Oxs_Ext::Error(this,
	   "Anv_SpinTEvolve3D::Step; Programming error:"
  ...
  if(!cstate.GetDerivedData("dE/dt",current_dE_dt)) {
    throw Oxs_Ext::Error(this,"Anv_SpinTEvolve3D::Step:"
...
void Anv_SpinTEvolve3D::UpdateDerivedOutputs(const Oxs_SimState& state)
{ // This routine fills all the Anv_SpinTEvolve3D Oxs_ScalarOutput's to
...
void Anv_SpinTEvolve3D::UpdateSpinTorqueOutputs(const Oxs_SimState& state)
{
  ...
  if(spin_torque_output.GetCacheRequestCount()>0) { //BRIAN: We will have to adjust this code like we did in the loop to calculate the LLG equation (It is almost identical code) 
