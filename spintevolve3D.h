...
#ifndef _ANV_SPINTEVOLVE3D
#define _ANV_SPINTEVOLVE3D
...
/* End includes */
class Anv_SpinTEvolve3D:public Oxs_TimeEvolver {
private:
  mutable OC_UINT4m mesh_id; 
	Oxs_OwnedPointer<Oxs_VectorField> u_init; //BRIAN: We must change this to a vector field Oxs_VectorField
	
	//spin-torque parameter
  mutable Oxs_MeshValue<ThreeVector> u; //BRIAN: We probably have to change this as well, although I'm not sure what OC_REAL8m is. I think it might be a mesh property declaring a scalar. 
									  // I think we need to replace OC_REAL8m with ThreeVector based off of the TimeEvolver code 
  ...
  Oxs_ScalarOutput<Anv_SpinTEvolve3D> max_dm_dt_output;
  Oxs_ScalarOutput<Anv_SpinTEvolve3D> dE_dt_output;
  Oxs_ScalarOutput<Anv_SpinTEvolve3D> delta_E_output;
  Oxs_ScalarOutput<Anv_SpinTEvolve3D> ave_u_output; //BRIAN: Change this to vector ??? No
  Oxs_VectorFieldOutput<Anv_SpinTEvolve3D> dm_dt_output;
  Oxs_VectorFieldOutput<Anv_SpinTEvolve3D> mxH_output;
  Oxs_VectorFieldOutput<Anv_SpinTEvolve3D> spin_torque_output;
...
#define RKStepFuncSig(NAME) \
  ...
  RKStepFuncSig((Anv_SpinTEvolve3D::* rkstep_ptr));
  ...
  Anv_SpinTEvolve3D(const Anv_SpinTEvolve3D&);
  Anv_SpinTEvolve3D& operator=(const Anv_SpinTEvolve3D&);

public:
  virtual const char* ClassName() const; // ClassName() is
  /// automatically generated by the OXS_EXT_REGISTER macro.
  virtual OC_BOOL Init();
  Anv_SpinTEvolve3D(const char* name,     // Child instance id
		       Oxs_Director* newdtr, // App director
		       const char* argstr);  // MIF input block parameters
  virtual ~Anv_SpinTEvolve3D();
